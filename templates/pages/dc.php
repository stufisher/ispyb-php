                ...
                <?php foreach ($vbd[($i-$first+1)] as $t => $vi): ?>
                    <li><span class="time"><?php echo $t ?></span>
                        ...
                        <li class="<?php echo (time() > strtotime('23:59 '.($i - $first +1).'-'.$c_month.'-'.$c_year)) ? 'past' : '' ?>">
                            <?php echo $v['BL'] ?> - <?php if ($v['REG']): ?><a class="bold" href="/dc/visit/<?php echo $v['VIS'] ?>"><?php echo $v['VIS'] ?></a><?php else: ?><?php echo $v['VIS'] ?><?php endif; ?> (<?php echo number_format($v['LEN'],0) ?>h)
                            <?php if ($v['LC']): ?>
