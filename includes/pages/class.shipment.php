    ...
    class Shipment extends Page {
        ...
        function _add_container() {
            ...
            if ($this->has_arg('submit')) {
                ...
                $shipid = $this->db->pq("SELECT shippingid FROM ispyb4a_db.dewar WHERE dewarid=:1", array($this->arg('did')));
                $shipid = sizeof($shipid) ? $shipid[0]['SHIPPINGID'] : 0;
                
                $this->msg('New Container Added', 'Your container was sucessfully added. Click <a href="/shipment/cid/'.$cid.'">here</a> to see to the container. Click <a href="/shipment/addc/did/'.$this->arg('did').'">here</a> to add another container to the same shipment, or click <a href="/shipment/sid/'.$shipid.'">here</a> to view the shipment');
