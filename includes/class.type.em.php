...
class EM extends ProposalType {
    ...
    var $visit_table = 'emmovie';
    var $session_column = 'blsessionid';
        ...
        // Normal validation
            ...
            // Normal users
                ...
                if ($this->has_arg('id') || $this->has_arg('visit') || $this->has_arg('prop')) {
                    ...
                    if ($this->has_arg('id')) {
                        $vis = $this->db->pq('SELECT p.proposalcode || p.proposalnumber || \'-\' || s.visit_number as vis FROM ispyb4a_db.blsession s INNER JOIN ispyb4a_db.proposal p ON (p.proposalid = s.proposalid) INNER JOIN ispyb4a_db.emmovie dc ON s.sessionid = dc.blsessionid WHERE dc.emmovieid = :1', array($this->arg('id')));
